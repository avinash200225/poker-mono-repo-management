...
import { 
  ...
  PLAYER_CREATED, 
  PLAYER_UPDATED,
  PLAYER_OFFLINE, PLAYER_ONLINE, PLAYER_LOCK, PLAYER_UNLOCK, PLAYER_LOCKED, PLAYER_UNLOCKED, ROUND_RESULT 
...
import handleRoundTransactions from "./handleRoundTransactions";
...
const socketEpic = (action$, state$) =>
  action$.pipe(
    ...
    switchMap(() => {
      ...
      const {
        REACT_APP_WS_HOST = `${window.location.host}`,
        REACT_APP_WS_PATH,
      } = window

      const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
      // const url = `${protocol}//${loc.host}/poker/wsclient/topper`
      const url = `${protocol}//${REACT_APP_WS_HOST}${REACT_APP_WS_PATH}`
        const initMessage = { MessageType: 'INITIALIZE_ADMIN', ClientIp: "192.168.1.200" }
...
function dataUpdatedEpic(action$, state$) {
  return action$.pipe(
    ofType(socket.message),
    pluck('payload'),
...
function RoundTransactionsEpic(action$, state$) {
  return action$.pipe(
    ofType(socket.message),
    pluck('payload'),
    filter(({ MessageType, }) => (MessageType === ROUND_RESULT)),
    mergeMap((socketMessage) => handleRoundTransactions(socketMessage, state$.value))
  );
}

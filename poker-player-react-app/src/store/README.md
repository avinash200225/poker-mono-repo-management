...
## redux-obeservable(1.2.0)
    ...
    - ofType
    ~~~javascript
    const socketConnectEpic = (action$, state$) => action$.pipe(
        ofType(socket.request),
        switchMap(() => {
            //1. Pluck Socket connection url
            //2. Pluck input data for inital data request message
            //
        }),
        catchError(() => {
            //Send errorAction.error.set()
        })

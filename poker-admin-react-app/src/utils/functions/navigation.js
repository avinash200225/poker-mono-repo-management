...
export function openPopup(url, _ref) {
  ...
  // handle dual screens
  var dualScreenLeft =
    window.screenX !== undefined ? window.screenX : screen.left;
  var dualScreenTop =
    window.screenY !== undefined ? window.screenY : screen.top;
...
export function navigate(method, url) {
  ...
  var _url = options.queryString
    ? baseUrl
        .concat(baseUrl.indexOf("?") === -1 ? "?" : "&")
        .concat(options.queryString)
    : baseUrl;

  /* eslint-disable max-len */
  if (
    method === NAVIGATION_METHODS.LOBBY ||
    method === NAVIGATION_METHODS.URL ||
    method === NAVIGATION_METHODS.REPLACE ||
    (method === NAVIGATION_METHODS.POPUP && !!window.opener)
  ) {
